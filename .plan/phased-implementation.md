...
P18.7 note: `ONE-119` started on 2026-05-13 after `ONE-118` merged and Linear was synced. The final Phase 18 review gate reconciled P18.1 through P18.6 against the AI planner objective, REST/security requirements, demo/live behavior, consent gates, schema validation, editable draft flow, local Travelpayouts handoff intent boundary, and Phase 19 readiness. Focused backend validation passed 34 assertions for explicit route permission callbacks, unauthenticated/run-only failures, demo draft save, live missing-key/unsupported-provider/missing-consent failures, provider-claim schema rejection, handoff nonce/capability/approval/consent failures, local `not_executed` handoff success, malformed handoff argument rejection, session privacy, and cleanup. Playwright Chromium runtime validation was used after the Codex in-app Browser connection timed out; screenshots and keyboard review confirmed `/trip-planner/` page identity, nonblank content, demo itinerary `201`, saved draft result, handoff `201`, local `Provider action: not_executed`, live missing per-request consent with zero additional itinerary REST requests, no fake key or raw private prompt sentinel rendered/stored, no app-owned console/request failures, no desktop/mobile overflow, keyboard reachability through planner controls and handoff controls, and cleanup of temporary user/draft content. No app-owned production-code bug was found in the final gate. PR #55 was reviewed by Codex with no major issues, had no unresolved review threads, and merged into `main` at `ac6e2669e4d7c64c7911884e5b1defe0d47f0fe3`.
...
P19.2 note: `ONE-121` started on 2026-05-13 after `ONE-120` merged and Linear was synced. The existing P15.5 alert capture flow now has a dedicated local lifecycle service for deduping email/route intents, capping each email at ten active or pending alerts, recording email follow-up status/timestamps, and processing pending confirmation/manage emails through the hourly `baf_process_travel_alerts` cron hook. Legacy duplicate requested rows for the same email/route send one follow-up and mark later queued duplicates skipped. Failed `wp_mail()` sends move alerts to `email_failed` so delivery failures do not permanently consume the active/pending quota, while later same-email/same-route saves can queue another attempt. Alert emails keep provider-owned live fare, booking, payment, changes, and support language visible, and include a signed delete-confirmation link that opens the Flights page and requires an alert-specific nonce-protected POST before the private `travel_alert` record is permanently removed. No plaintext delete token is stored, and deletion only succeeds when WordPress returns the deleted post object. The homepage and Flights alert form copy now describe the active local alert intent workflow instead of staged preview language. No Travelpayouts provider call, live fare inventory, booking, payment, raw prompt storage, provider payload logging, or REST exposure was added.

P19.3 note: `ONE-122` started on 2026-05-13 after `ONE-121` merged and Linear was synced. The core plugin now registers WordPress personal-data exporters and erasers for saved trips, travel alerts, AI trip-plan drafts, and AI session logs. Saved-trip privacy exports include safe local Flights, Hotels, and Trip Planner resume links; alert exports include a local Flights price-alert resume link; AI trip-plan exports include a permission-gated local draft link. Erasure permanently removes matching saved trips, alerts, and AI trip-plan drafts, and anonymizes AI session rows by clearing user/source linkage, run UUID, request/output hashes, output summary, and error message. The local runtime pass confirmed saved-trip resume links, planner prefill, alert resume route prefill, desktop/mobile screenshots, and keyboard reachability of saved-trip action links. The implementation does not export raw AI prompts, itinerary JSON, provider payloads, request/output hashes, booking/payment identifiers, confirmation data, provider secrets, or live inventory.

P19.4 note: `ONE-123` started on 2026-05-13 after `ONE-122` merged and Linear was synced. The admin Reports screen now exposes a Travelpayouts SubID reporting map for the major Bookings and Flights surfaces and an observed local SubID click table from signed handoff events. The map uses the existing placement registry and SubID builder where possible, adds mapped-only rows for planned AI/saved-trip/alert reporting surfaces, and keeps SubIDs limited to placement context. Reports and CSV export now state that WordPress-local clicks are operational analytics only, while Travelpayouts Performance reports remain the source of truth for partner clicks, searches, bookings, conversion, and earnings. Provider stats with unavailable metric values render as `Unavailable` rather than implying a zero value. Runtime validation used Playwright Chromium after the Codex in-app Browser connection timed out; desktop/mobile screenshots and keyboard review confirmed the Reports page renders the map, local rows, unavailable state, no app-owned console/request failures, and no horizontal overflow.

P19.5 note: `ONE-124` started on 2026-05-13 after `ONE-123` merged and Linear was synced. The admin Reports flow now has explicit privacy guardrail rows in the dashboard and CSV export for the report capability gate, approved date windows, bounded result sets, private-data exclusions, and Travelpayouts Performance reports as the provider revenue/conversion source of truth. Report-only users with `view_baf_reports` but without settings/widget-placement capabilities now land on Reports from the top-level Bookings & Flights admin menu rather than an inaccessible dashboard. Invalid `days` requests still fall back to 30 days. Local WP-CLI validation confirmed no-cap users are denied, report-only users can render Reports, guardrail export rows exist, and forbidden internal fields are absent. Runtime validation used Playwright Chromium after the Codex in-app Browser connection timed out; a report-only user pass confirmed desktop/mobile rendering, skip-link keyboard reachability to report controls, report-only menu visibility, no app-owned console/request failures, and no horizontal overflow.

P19.6 note: `ONE-125` started on 2026-05-14 after `ONE-124` merged and Linear was synced. The full security/source exposure pass reviewed core REST controllers, settings, admin screens, frontend shortcodes/routes, AI provider wiring, alert handlers, reports, active affiliate bridge routes, tracked Travelpayouts REST/widget routes, and public/admin source output. Runtime route inventory confirmed explicit permission callbacks on all concrete app-owned `baf/v1` routes and tracked Travelpayouts action routes; the only missing callback entries were WordPress namespace-index discovery records from `WP_REST_Server::get_namespace_index`. Fake Travelpayouts, AI, affiliate bridge, and postback secrets were absent from protected admin HTML, public homepage source, public affiliate bridge config output, and the Travelpayouts token endpoint. Playwright Chromium runtime validation was used after the Codex in-app Browser reported no active pane; desktop admin, mobile homepage, and config screenshots plus skip-link keyboard review passed with no app-owned console/request failures.

P19.7 note: `ONE-126` started on 2026-05-14 after `ONE-125` merged and Linear was synced. The performance, accessibility, responsive, and browser regression pass covered public travel pages, authenticated saved trips, and protected Bookings & Flights admin screens. The pass found and fixed early cron scheduling that could print a WordPress `_load_textdomain_just_in_time` notice before public widget output, search submissions that did not move users directly to the configured provider section with the submitted intent visible, missing Settings API `label_for` metadata, duplicate Widget Placements delete-form nonce IDs, and strict target-size misses on app-owned checkboxes. `Cron_Manager::schedule_events()` now runs on `init` priority 20; home/Flights/Hotels search-surface forms target `#flights-provider-search` or `#hotels-provider-search`, highlight submitted provider sections, normalize route codes, and pass flight route hints into the Travelpayouts White Label configuration; Settings fields share a `label_for` helper; placement delete nonces use per-placement field names; and scoped checkbox/button targets meet the strict browser audit. The Codex in-app Browser path still reported no active pane, so Playwright Chromium captured 27 screenshots and 8 keyboard paths; `/tmp/one126-runtime-review-report-final.json` returned `status=pass` and `findingCount=0` after separating provider-owned Travelpayouts/Trip.com/pixel/WebGL noise from app-owned failures.

P19.8 note: `ONE-127` started on 2026-05-14 after `ONE-126` merged and Linear was synced. The final launch-readiness gate reconciled Phase 11 through Phase 19 against the Travelpayouts-first backend boundary, the launch checklist, current known issues, regression watchlist, validation baseline, and architecture baseline. No app-owned local launch blocker remains after the gate. Current sanity checks confirmed concrete core `baf/v1` routes have callable permission callbacks, Phase 5 cron hooks are scheduled, custom tables are present, privacy exporter/eraser hooks are registered, Flights/Hotels return HTTP 200, public Flights/Hotels/Trip Planner/Saved Trips source scans show no PHP warning/fatal/deprecated/textdomain output or credential patterns, `bookings-flights-core` deactivate/reactivate/is-active passes, and all tracked core/static theme source files are at or below 600 lines. The production launch watch remains environment-specific: validate real Travelpayouts account/domain/White Label settings, partner embed behavior, and deployed browser/content-security conditions before public go-live. PR review and merge remain the final project-tracking gate before Linear `ONE-127` and parent `ONE-67` are moved to Done.
