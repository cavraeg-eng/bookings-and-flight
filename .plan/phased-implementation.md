...
P19.2 note: `ONE-121` started on 2026-05-13 after `ONE-120` merged and Linear was synced. The existing P15.5 alert capture flow now has a dedicated local lifecycle service for deduping email/route intents, capping each email at ten active or pending alerts, recording email follow-up status/timestamps, and processing pending confirmation/manage emails through the hourly `baf_process_travel_alerts` cron hook. Alert emails keep provider-owned live fare, booking, payment, changes, and support language visible, and include a signed delete link that permanently removes the private `travel_alert` record without storing plaintext delete tokens. The homepage and Flights alert form copy now describe the active local alert intent workflow instead of staged preview language. No Travelpayouts provider call, live fare inventory, booking, payment, raw prompt storage, provider payload logging, or REST exposure was added.
