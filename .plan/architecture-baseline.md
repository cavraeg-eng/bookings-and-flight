...
- P19.4 SubID reporting map: `BAF\Core\Services\Travelpayouts_Subid_Map_Service` maps major home, Flights, Hotels, destination guide, route guide, AI planner, saved-trip, and alert surfaces to readable Travelpayouts SubID examples for admin reporting. Local `bf_clicks` SubID rows are privacy-aware operational signals only; Travelpayouts Performance reports remain the source of truth for partner searches, clicks, bookings, conversion, and earnings. Missing or null provider metrics must render as unavailable, not as zero revenue or zero conversions.
- P19.8 launch-readiness result: the Travelpayouts-first source-of-truth boundary remains the release baseline. No app-owned local launch blocker remains after Phase 19 validation, but production go-live still requires deployed-environment validation of the real Travelpayouts account/domain, White Label configuration, partner embeds, caching/content-security, consent settings, and target browser mix.
...
Phase 19.3 adds WordPress personal-data exporter and eraser hooks through `BAF\Core\Privacy\Personal_Data_Manager`. Exporters cover member-owned saved trips, email-owned travel alerts, AI-generated trip plan drafts, and AI session logs. Saved-trip exports include local Flights, Hotels, and Trip Planner resume links; alert exports include a local Flights price-alert resume link; AI trip-plan exports include a local WordPress draft link when available. Erasers permanently delete matching saved-trip records, alert records, and AI trip-plan drafts, while AI session rows are anonymized by clearing user linkage, source post linkage, run UUID, request/output hashes, output summary, and error message. Privacy exports do not include raw AI prompts, itinerary JSON payloads, request/output hashes, provider payloads, API keys, provider secrets, booking IDs, payment data, confirmation numbers, or live inventory.
...
Frontend placement rendering is implemented by `BAF\Core\Frontend\Travelpayouts_Widget_Renderer`, `[baf_travelpayouts_widget]`, and the dynamic `baf/travelpayouts-widget` block. Shortcode and block attributes store only placement key, surface, channel, slug, optional class data, and safe route intent such as `origin`/`destination`; raw embed references and URLs stay in the registry option and are read only by trusted server-side rendering. `BAF\Core\Services\Travelpayouts_Widget_Subid_Service` owns the runtime SubID convention `{channel}_{surface}_{vertical}_{slug}_{placement}`, normalizes generated SubIDs to lowercase Latin letters, numbers, and underscores, and mutates provider URLs without dropping existing Travelpayouts partner markers. Public output may contain approved provider script or iframe URLs as rendered widget markup, and the White Label configuration may receive sanitized `flightParams.origin` / `flightParams.destination` hints, but public REST-style placement projections and editor attributes must not expose private `embed.reference`, private `embed.url`, admin notes, private saved-trip data, emails, payment data, or unsupported provider-owned filter claims. Public widget frames expose safe state metadata through `data-baf-state` and `data-baf-render-mode`; consent-disabled, disabled, missing-configuration, loading, no-script, unavailable, and configured states must render escaped copy and must not print provider output when consent or configuration is missing. Existing `[baf_travelpayouts_white_label]` and `[baf_travelpayouts_hotel_widget]` shortcodes remain available for validated setup checks and backward compatibility; new page/template work should prefer `[baf_travelpayouts_widget placement="flights_white_label_search"]`, `[baf_travelpayouts_widget placement="hotels_partner_search"]`, or the governed hotel companion placement keys.
...
Phase 16.1 confirms the first Hotels surface consumes the existing `hotels_partner_search` placement key and adds no new registry schema version, provider contract, REST route, option, table, cron hook, shortcode, or block. Hotel intent query parameters are local display state only. As of P19.7, the shared search-surface behavior preserves submitted intent in the URL, anchors home/Flights/Hotels submissions to the relevant provider section, highlights that provider target, and keeps WordPress-owned intent summaries visible before provider-owned search controls.
...
`BAF\Core\Plugin` schedules core cron events on `init` priority 20, not during plugin bootstrap, so WP-Cron registration does not print early textdomain notices or perform page-output-affecting work before WordPress initialization is ready.
...
- Phase 18 AI planner entry: P18.1 adds the real `/trip-planner/` route and prompt-to-trip-brief flow while keeping AI opportunities recommendation-only and blocking live provider requests without saved plus per-request consent. P18.5 aligns frontend live readiness and backend provider selection through `Provider_Factory::live_readiness()` so known-misconfigured live states do not send prompt data from the browser. P18.6 confirms the AI privacy/security review boundary: REST routes keep explicit permission callbacks, malformed non-scalar schema/provider/settings values must normalize safely before sanitization, dashboard/admin output must not cast or expose secrets, rendered planner output must not echo raw private prompts, and AI session/provider errors must not leak fake keys, bearer tokens, provider payloads, or raw prompts. P18.7 confirms the final Phase 18 boundary for Phase 19: local handoff intents are approved WordPress metadata only, not provider execution; Phase 19 may consume them only through new capability, consent, approval, validation, and documentation gates.
- Phase 19 security exposure pass: P19.6 confirms the current release-readiness route/source boundary. Concrete `baf/v1` routes must keep explicit permission callbacks; intentionally public endpoints must still have explicit callbacks and bounded validation. WordPress namespace-index discovery records from `WP_REST_Server::get_namespace_index` can appear in route inventories without app callbacks and should be classified separately from concrete route handlers. Admin and frontend output must not render provider API tokens, AI API keys, affiliate bridge tokens, postback secrets, raw prompts, bearer tokens, provider payloads, booking IDs, payment data, confirmation data, or live inventory. The local affiliate bridge and content manager plugins remain active runtime context but untracked source until a separate tracking decision is made.
...
- Integration status payloads expose the configured browser-reachable API origin from `PUBLIC_API_BASE`, falling back to local `http://localhost:4050` only for local development. Frontend integration CTAs should use that runtime value instead of hardcoded localhost links.
