...
Phase 16.6 confirms the final Hotels gate adds no new registry schema version, provider contract, REST route, option, table, cron hook, shortcode, block, or custom inventory backend. The only code contract change is the SEO metadata helper recognizing transient hotel-intent query parameters on `/hotels/` for `wp_robots` noindex/follow output, matching the existing transient flight-query boundary.
