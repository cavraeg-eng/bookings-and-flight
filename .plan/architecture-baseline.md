...
Phase 19.2 adds `admin-post.php` delete actions `baf_delete_flight_alert` / `admin_post_nopriv_baf_delete_flight_alert` for signed alert-delete links, plus bounded cron/email processing for requested alert intents. Email follow-up remains local alert-management messaging only; it does not call Travelpayouts, store live fare inventory, create provider links beyond the existing `/flights/` handoff, book, pay, or send private prompt/provider data.
