...
- WordPress.org still warns that `travelpayouts` has not been tested with the latest three major WordPress releases, so plugin-first placement is cleared only for the specific flight widget surface tested in P11.3. Hotel and White Label production placements still need fallback validation or real configured-domain validation before release.
