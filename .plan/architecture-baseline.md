...
- P11.5 backend mode decision: Phase 11 final mode is official-plugin-first only for locally validated flight widgets/search surfaces, with Travelpayouts dashboard-generated hotel widget/table/embed and White Label Widget/Page code wrapped by the future capability-gated `baf` placement registry when the official plugin path is unavailable or not fully configured.
...
- P11.6 local review result: Phase 11 passed the local evidence gate on 2026-05-12 with no custom inventory backend, no direct checkout/payment flow, no secret exposure in the tested frontend source, and no new WordPress REST route, database table, or migration. Phase 11 remains `In Review` until dashboard-generated hotel fallback embeds and real White Label Widget/Page configuration are validated with Travelpayouts-provided setup.
