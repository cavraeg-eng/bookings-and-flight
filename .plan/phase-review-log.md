...
Bugs fixed: The validation command now uses the Local MySQL socket. The shared placement part now sanitizes multi-class strings class-by-class. Link-only hotel companion cards now use compact body sizing while future map/listing iframes remain protected by placement-specific frame metadata. Browser and keyboard checks were rerun after the layout fix.
