...
Validation performed: PHP syntax checks for changed PHP files; file-size review after splitting shared helpers into `class-personal-data-records.php` (`class-personal-data-manager.php` 325 lines, `class-personal-data-records.php` 324 lines); `git diff --check`; `bookings-flights-core` deactivate/reactivate/is-active checks with the LocalWP PHP/MySQL socket config; focused WP-CLI privacy smoke with 54 assertions for exporters, erasers, cross-user denial, anonymization, unrelated-user preservation, export omissions, and cleanup; retained-record erasure smoke with 31 assertions for continued pagination when a full first saved-trip batch cannot be deleted; published-AI privacy smoke with 5 assertions proving published trip-plan posts stay out of draft export/erase scope; Playwright Chromium runtime screenshots and keyboard review for saved-trip resume links, planner prefill, alert route prefill, desktop/mobile overflow, source-secret checks, app-owned request health, and cleanup.

Bugs found: The first privacy smoke assertions were too literal about JSON-escaped URLs, so the temporary validation harness was tightened. Browser passes surfaced only known provider-owned widget console noise: React JSX-source warnings, `tpembars.com` CORS/config messages, and generic provider/widget resource failures, while app-owned request checks passed.

Bugs fixed: Added `BAF\Core\Privacy\Personal_Data_Manager`, registered it from the core bootstrap, added `BAF\Core\Privacy\Personal_Data_Records` for shared privacy record helpers, added the privacy directory guard file, scoped export/erase callbacks across saved trips, alerts, AI drafts, and AI sessions, scrubbed AI session personal linkage during erasure, kept erasure pagination active when records are retained by storing retained IDs during the request and excluding them on later callbacks, restricted AI trip-plan privacy export/erase to draft/private non-public records, and updated validation scripts to assert link presence without depending on JSON URL slash escaping.
