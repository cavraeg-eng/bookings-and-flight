...
Validation performed: PHP syntax checks for changed PHP files; file-size review after splitting shared helpers into `class-personal-data-records.php` (`class-personal-data-manager.php` 310 lines, `class-personal-data-records.php` 280 lines); `git diff --check`; `bookings-flights-core` deactivate/reactivate/is-active checks with the LocalWP PHP/MySQL socket config; focused WP-CLI privacy smoke with 54 assertions for exporters, erasers, cross-user denial, anonymization, unrelated-user preservation, export omissions, and cleanup; retained-record erasure smoke with 31 assertions for continued pagination when a saved-trip record cannot be deleted; published-AI privacy smoke with 5 assertions proving published trip-plan posts stay out of draft export/erase scope; Playwright Chromium runtime screenshots and keyboard review for saved-trip resume links, planner prefill, alert route prefill, desktop/mobile overflow, source-secret checks, app-owned request health, and cleanup.
