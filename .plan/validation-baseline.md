...
P14.2 Codex review accessibility follow-up on 2026-05-12: PR #23 review found that the loaded White Label placeholder nodes stayed `aria-hidden` after the provider script loaded. The renderer now leaves loaded placeholders at `tabindex="-1"` to avoid duplicate sequential keyboard stops but removes `aria-hidden` so injected provider content remains exposed to assistive technology. PHP syntax, `git diff --check`, and a Playwright runtime check for `#tpwl-search` loaded state passed.
