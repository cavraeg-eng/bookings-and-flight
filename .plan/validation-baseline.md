...
P19.5 local result on 2026-05-14: focused WP-CLI permission smoke confirmed no-cap users are denied by `Reports_Page::render()`, a subscriber with only `view_baf_reports` gets a report-capability top-level Bookings & Flights menu route, invalid `days=999` falls back to the 30-day window, report guardrail rows render, forbidden internal fields such as request/output hashes and API keys are absent, and CSV export rows include at least five `report_guardrail` rows. Playwright Chromium runtime validation was used after the Codex in-app Browser connection timed out; a report-only user pass confirmed desktop/mobile Reports screenshots, skip-link keyboard reachability through the date selector, Apply button, and Export CSV link, report-only menu visibility, no app-owned console/request failures, and no horizontal overflow. Temporary report-only users were deleted after validation.
...
P19.1 local result on 2026-05-13: PHP syntax passed for changed saved-trip service, REST controller, frontend route, template, post-type registrar, REST/frontend managers, AI planner prefill, and theme files. `node --check` passed for `saved-trips.js` and `ai-planner.js`. File-size review passed; the largest new source file is `class-saved-trip-service.php` at 495 lines before the Codex-review handoff patch, and the existing static theme `functions.php` remains exactly 600 lines with only an in-place URL change. WP-CLI saved-trip REST smoke passed 25 assertions for route registration, explicit permission callbacks, anonymous denial, nonce denial, consent denial, valid save/list/update/delete, cross-user denial, invalid date rejection, resume payload, minimized storage, and cleanup. Code review also patched partial item-update merge behavior so omitted fields are preserved while local-storage consent remains required. Codex PR review then patched saved-trip flight handoff normalization so valid three-letter codes populate `origin`/`destination`, while free-form saved route labels are preserved through `travel_origin`/`travel_destination`; WP-CLI and Playwright handoff checks confirmed free-form `Miami to Lisbon` remains visible without invalid code-field prefill and valid IATA entries still pass as codes. The Codex in-app Browser connection timed out after 15 seconds, so Playwright Chromium was used for runtime screenshots and keyboard review. Runtime evidence is saved at `/tmp/one120-runtime-review-report.json`; screenshots include `/tmp/one120-saved-trips-logged-out.png`, `/tmp/one120-saved-trips-empty.png`, `/tmp/one120-saved-trips-saved.png`, `/tmp/one120-saved-trips-planner-resume.png`, and `/tmp/one120-saved-trips-mobile.png`. The final report returned `status=pass` and `findingCount=0`, with no relevant app-owned console errors, failed requests, horizontal overflow, blank page, framework overlay, saved-card secret/payment/booking strings, or cleanup residue. Follow-up fixes addressed saved-route header contrast, mobile admin-bar overlap, and exposed the AI planner resume link on saved-trip cards.

## Phase 19 Alert Intent Validation

Use this when changing `[baf_flight_alert_signup]`, local `travel_alert` storage, alert email follow-up, or `baf_process_travel_alerts`.

```bash
php -l plugins/bookings-flights-core/includes/services/class-flight-alert-service.php
php -l plugins/bookings-flights-core/includes/frontend/class-flight-alert-intent-handler.php
php -l plugins/bookings-flights-core/includes/frontend/class-flight-alert-signup-shortcode.php
php -l plugins/bookings-flights-core/includes/jobs/class-job-runner.php
php -l plugins/bookings-flights-core/includes/cron/class-cron-manager.php
php -l plugins/bookings-flights-core/includes/post-types/class-post-type-registrar.php
php -l themes/bookings-and-flights-static/page-home.php
wp eval-file /tmp/one121-alert-smoke.php
node /tmp/one121-runtime-review.mjs
wp plugin deactivate bookings-flights-core
wp plugin activate bookings-flights-core
wp plugin is-active bookings-flights-core
git diff --check
...
P19.2 local result on 2026-05-13: PHP syntax passed for the new alert service, alert handler, alert shortcode, frontend manager, job runner, cron manager, post-type registrar, and homepage template. CSS file-size review passed after moving the alert-specific styles into `assets/css/flight-alert.css`. `bookings-flights-core` deactivate/reactivate/is-active passed with known WP-CLI PHP 8.5 deprecation noise only. Focused WP-CLI smoke passed 77 assertions covering initial alert save, duplicate email/route update, ten-alert per-email cap, eleventh-route rejection, queued email capture through `pre_wp_mail`, provider-boundary email copy, signed delete-confirmation URL generation, GET-delete non-destruction, `wp_delete_post()` false-return handling, static delete nonce rejection, alert-scoped nonce-confirmed POST deletion, cron job email processing/status recording, legacy duplicate queued-alert dedupe, failed `wp_mail()` transition to `email_failed`, failed-email exclusion from the active/pending quota, same-route retry after an email failure, quota query-cache freshness, shortcode lifecycle copy, nonce rendering, and temporary alert cleanup. A cached-offer cron import smoke returned `success`, confirming the `Post_Type_Registrar` import remained available to the existing cached-offer path. A real missing-nonce `admin-post.php` request returned `403` and created zero alert records. New alert follow-up meta keys were confirmed registered for `travel_alert` with `show_in_rest=false`; the meta-registration probe emitted duplicate core block registration notices only because it manually invoked `init` inside WP-CLI after bootstrap. The Codex in-app Browser path was attempted first, but no active browser pane was available, so Playwright Chromium was used. Runtime evidence is saved at `/tmp/one121-runtime-review-report.json` and `/tmp/one121-delete-runtime-review-report.json`; screenshots include `/tmp/one121-alert-form-desktop.png`, `/tmp/one121-alert-saved-desktop.png`, `/tmp/one121-alert-form-mobile.png`, and `/tmp/one121-alert-delete-confirm-desktop.png`. Both reports returned `status=pass` and `findingCount=0`, confirming the alert form renders lifecycle/provider-boundary copy, real browser POST submission redirects to `baf_alert_status=saved`, the success state mentions queued confirmation/delete email, the delete confirmation form is keyboard reachable and submits through POST, no app-owned console errors or failed requests were observed, no horizontal overflow was found, no secret-like source text was rendered, and temporary `one121-*` alerts were cleaned before final smoke reruns.

## Phase 19 Privacy Export/Erase Validation

Use this when changing WordPress personal-data exporter/eraser hooks, saved-trip export data, alert export data, AI trip-plan export data, or AI session anonymization.

```bash
php -l plugins/bookings-flights-core/includes/class-plugin.php
php -l plugins/bookings-flights-core/includes/privacy/class-personal-data-manager.php
php -l plugins/bookings-flights-core/includes/privacy/class-personal-data-records.php
php -l plugins/bookings-flights-core/includes/privacy/index.php
wp eval-file /tmp/one122-privacy-smoke.php
wp eval-file /tmp/one122-retained-erasure-smoke.php
wp eval-file /tmp/one122-ai-public-privacy-smoke.php
node /tmp/one122-runtime-review.mjs
wp plugin deactivate bookings-flights-core
wp plugin activate bookings-flights-core
wp plugin is-active bookings-flights-core
git diff --check
...
P19.3 local result on 2026-05-13: PHP syntax passed for the core plugin bootstrap and new privacy module files. File-size review passed after splitting shared helpers into `class-personal-data-records.php`; `class-personal-data-manager.php` is 325 lines and `class-personal-data-records.php` is 324 lines. Focused WP-CLI privacy smoke passed 54 assertions covering exporter/eraser registration, owner-scoped saved-trip export, alert email-scoped export, AI trip-plan export, AI session export, cross-user saved-trip resume denial, saved-trip erasure, alert erasure, AI draft erasure, AI session anonymization, unrelated-user preservation, and absence of raw prompt/hash sentinels from exports. A follow-up retained-record erasure smoke passed 31 assertions covering a 26-record saved-trip batch where the first 25 posts are retained, confirming the eraser reports `items_retained`, keeps `done=false` for the retained first page, stores retained IDs, and continues processing later records on the next callback. A published-AI privacy smoke passed 5 assertions confirming published trip-plan posts with `baf_ai_source_session_id` stay out of draft privacy export/erase scope and are not deleted. `bookings-flights-core` deactivate/reactivate/is-active passed with known WP-CLI PHP 8.5 and third-party Travelpayouts deprecation noise only; WP-CLI was run with the LocalWP PHP/MySQL socket config for this site. `git diff --check` passed. The Codex in-app Browser connection timed out after 15 seconds, so Playwright Chromium was used for real runtime validation. Runtime evidence is saved at `/tmp/one122-runtime-review-report.json`; screenshots include `/tmp/one122-saved-trips-resume-links-desktop.png`, `/tmp/one122-planner-resume-desktop.png`, `/tmp/one122-alert-resume-flight-desktop.png`, and `/tmp/one122-saved-trips-mobile.png`. The runtime report returned `status=pass` and `findingCount=0`, confirming saved-trip Flight/Hotel/AI Planner resume links route to local surfaces, planner resume prefill works, the alert price-watch resume URL pre-fills route fields on `/flights/`, saved-trip action links are keyboard reachable, desktop/mobile pages have no horizontal overflow, no app-owned failed requests or source-secret strings were observed, and temporary browser users/posts were cleaned up. Provider-owned Travelpayouts JSX-source, `tpembars.com` CORS/config, and widget-resource console noise remained watchlist-only because app-owned requests and page behavior passed.

## Phase 19 Security Exposure Validation

Use this when changing REST routes, admin settings, affiliate bridge integration, provider settings, reports, AI provider settings, saved-trip endpoints, alert handlers, widget placement output, or launch-readiness security docs.

```bash
rg -n "register_rest_route|permission_callback|admin_post_|wp_ajax_|wp_ajax_nopriv|add_shortcode|check_admin_referer|wp_verify_nonce|current_user_can|\\$_(GET|POST|REQUEST|SERVER|COOKIE)" plugins/bookings-flights-core plugins/travelpayouts themes/bookings-and-flights-static -g "*.php"
rg -n "api[_-]?key|access[_-]?token|authorization|bearer|secret|password|client[_-]?secret|account_api_token|postback|sk-[A-Za-z0-9]|token" plugins/bookings-flights-core plugins/travelpayouts themes/bookings-and-flights-static -g "*.php" -g "*.js" -g "*.json" -g "!plugins/travelpayouts/assets/*.js" -g "!plugins/travelpayouts/assets/*.css"
wp plugin list --status=active --fields=name,status,version
wp eval runtime REST route inventory for `baf/v1`, `redux_travelpayouts`, and `travelpayouts/widget`
wp eval focused unauthenticated REST permission checks for public and protected `baf/v1` routes
wp eval fake-secret admin/public/config/Travelpayouts token exposure smoke with cleanup
node Playwright Chromium admin/public/config screenshot and keyboard/source review
wp plugin deactivate bookings-flights-core
wp plugin activate bookings-flights-core
wp plugin is-active bookings-flights-core
git diff --check
...
P19.8 local result on 2026-05-14: Launch-readiness validation reused the P19.7 real browser evidence and added fresh current-state sanity checks. `bookings-flights-core` deactivate/reactivate/is-active passed with the LocalWP MySQL socket. Core route inventory confirmed callable permission callbacks for `/baf/v1/destinations`, `/baf/v1/routes`, `/baf/v1/affiliate/click`, `/baf/v1/ai/itinerary`, `/baf/v1/ai/handoff`, and `/baf/v1/saved-trips`. Cron schedule checks confirmed `baf_refresh_cached_offers`, `baf_process_travel_alerts`, `baf_sync_provider_stats`, and `baf_cleanup_job_records` are scheduled. Custom table checks confirmed `bf_clicks`, `bf_ai_sessions`, and `bf_provider_stats` are present. Privacy hook checks confirmed `wp_privacy_personal_data_exporters` and `wp_privacy_personal_data_erasers` are registered. HTTP checks confirmed `/flights/` and `/hotels/` return `200`. Public source scans across `/flights/`, `/hotels/`, `/trip-planner/`, and `/saved-trips/` found no PHP warning/fatal/deprecated/parse/textdomain output and no API key, bearer, authorization, postback secret, access token, raw prompt, request hash, or output hash patterns. File-size scan found no tracked core plugin or static theme PHP/CSS/JS source file above 600 lines; `themes/bookings-and-flights-static/functions.php` is exactly 600 lines and should be split before further theme bootstrap expansion. `git diff --check` passed. WP-CLI emitted known PHP 8.5 deprecation warnings from bundled dependencies, and full active-plugin checks can still print known official Travelpayouts PHP 8.5 deprecations in CLI/admin contexts, but commands completed successfully.
...
P18.2 local result on 2026-05-13: PHP syntax passed for `plugins/bookings-flights-core/includes/services/class-ai-itinerary-service.php`, `plugins/bookings-flights-core/includes/frontend/class-ai-planner-page.php`, and `plugins/bookings-flights-core/templates/ai-planner-page.php`; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`. REST admin save smoke confirmed `save=true` creates a `draft` `trip_plan` with editable day content, itinerary JSON, departure meta, and no raw prompt echo, then deletes the draft. REST limited-user smoke confirmed a temporary user with `run_baf_ai` but no `edit_baf_content` receives `403:baf_ai_save_forbidden`. Playwright Chromium evidence is saved at `/tmp/one114-runtime-report.json` and `/tmp/one114-final-runtime-report.json`; screenshots include `/tmp/one114-planner-save-empty.png`, `/tmp/one114-planner-save-result.png`, `/tmp/one114-trip-plan-editor.png`, `/tmp/one114-final-planner-empty.png`, `/tmp/one114-final-planner-result.png`, and `/tmp/one114-final-trip-plan-editor.png`. The final runtime pass confirmed page identity, enabled editor save checkbox, keyboard focus through prompt, save option, and submit, `POST /wp-json/baf/v1/ai/itinerary` returning `201`, saved draft message, edit URL to `wp-admin/post.php?post=<id>&action=edit`, four day cards, two recommendation-only opportunities, no raw prompt echo, and no failed `baf/v1` responses. The final browser rerun caught and fixed a frontend submit regression caused by response payload variable shadowing. The Codex in-app Browser path still had no active pane, so Playwright Chromium was used. Temporary draft and temporary users were deleted after validation.

P18.3 local result on 2026-05-13: PHP syntax passed for `plugins/bookings-flights-core/includes/ai/class-itinerary-schema.php`, `plugins/bookings-flights-core/includes/ai/class-demo-ai-provider.php`, `plugins/bookings-flights-core/includes/ai/class-openai-provider.php`, and `plugins/bookings-flights-core/includes/services/class-ai-itinerary-service.php`; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`. Focused WP-CLI schema validation accepted a valid `travelpayouts_opportunity_v1` recommendation and rejected `booked` status, provider price claims, string `"false"` approval bypasses, string `"false"` disclosure bypasses, unsupported provider IDs, and non-scalar provider claim fields without `Array to string conversion` warnings. Codex review follow-up also scanned non-scalar `recommendation_type`, `label`, `suggested_subid`, `confidence`, `status`, `placement_context`, `limitations`, `requires_approval`, and `provider_link` cases without conversion warnings. Playwright Chromium evidence is saved at `/tmp/one115-runtime-report.json`; screenshots include `/tmp/one115-planner-empty.png`, `/tmp/one115-planner-result.png`, and `/tmp/one115-trip-plan-editor.png`. The runtime pass confirmed page identity, keyboard focus through prompt, save option, and submit, `POST /wp-json/baf/v1/ai/itinerary` returning `201`, rendered `flight_search` and `hotel_search` opportunities with safe suggested SubIDs and recommendation-only limitation copy, saved draft message, edit-link handoff, no raw prompt echo, no failed `baf/v1` responses, saved draft `baf_ai_opportunity_schema=travelpayouts_opportunity_v1`, and cleanup of temporary draft/user. The Codex in-app Browser path still had no active pane, so Playwright Chromium was used.

P18.4 local result on 2026-05-13: PHP syntax passed for `plugins/bookings-flights-core/includes/services/class-ai-opportunity-handoff-service.php`, `plugins/bookings-flights-core/includes/rest/class-ai-handoff-controller.php`, `plugins/bookings-flights-core/includes/rest/class-rest-manager.php`, `plugins/bookings-flights-core/includes/post-types/class-post-type-registrar.php`, and `plugins/bookings-flights-core/includes/frontend/class-ai-planner-page.php`; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`; `git diff --check` passed; changed source files remained below the 600-line guideline. `bookings-flights-core` deactivate/reactivate/is-active passed with known local WP-CLI PHP 8.5 deprecation noise only, and `curl -I http://bookings-and-flights.local/trip-planner/` returned `200 OK`. Focused WP-CLI REST/service smoke confirmed missing nonce returns `403:baf_ai_handoff_nonce_required`, limited-user access returns `403:baf_rest_forbidden`, missing approval returns `400:baf_ai_handoff_approval_required`, missing per-request provider consent returns `403:baf_ai_handoff_request_consent_required`, missing global provider consent returns `403:baf_ai_handoff_consent_required`, malformed non-scalar args return `400:rest_invalid_param` without app-owned PHP warnings, and the happy path returns `201` with one stored `draft_approved` intent, `provider_action=not_executed`, `external_request_sent=false`, no raw prompt sentinel, and no provider link. Browser validation attempted the Codex in-app Browser first and used Playwright Chromium fallback because no active pane was available. Playwright evidence is saved at `/tmp/one116-handoff-runtime-report.json`; screenshots include `/tmp/one116-handoff-empty.png`, `/tmp/one116-handoff-result.png`, `/tmp/one116-handoff-prepared.png`, and `/tmp/one116-handoff-mobile.png`. The runtime pass authenticated to `/trip-planner/`, confirmed keyboard focus through planner controls and handoff controls, submitted a saved draft, prepared the first local handoff, observed `POST /wp-json/baf/v1/ai/itinerary` and `POST /wp-json/baf/v1/ai/handoff` both returning `201`, displayed `Provider action: not_executed`, found no app-owned console errors or failed `baf/v1` responses, and cleaned up the temporary post/user after confirming stored handoff meta did not contain the raw prompt sentinel or provider links.

P18.5 local result on 2026-05-13: PHP syntax passed for `plugins/bookings-flights-core/includes/ai/class-provider-factory.php`, `plugins/bookings-flights-core/includes/ai/class-openai-provider.php`, `plugins/bookings-flights-core/includes/services/class-ai-itinerary-service.php`, `plugins/bookings-flights-core/includes/rest/class-ai-itinerary-controller.php`, `plugins/bookings-flights-core/includes/rest/class-request-parameters.php`, `plugins/bookings-flights-core/includes/frontend/class-ai-planner-page.php`, and `plugins/bookings-flights-core/templates/ai-planner-page.php`; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`. Focused WP-CLI REST/service smoke confirmed demo success, unauthenticated permission failure, run-only draft-save failure, missing per-request consent, missing API key, unsupported provider, missing saved External AI consent, malformed non-scalar request rejection, provider error safety, and AI session error safety. Fake API key and raw prompt sentinels were absent from provider/session errors. Follow-up malformed-option smoke confirmed array-valued AI provider settings fail safely as `503:baf_ai_not_configured` without `Array to string conversion` warnings. Browser validation attempted the Codex in-app Browser first and used Playwright Chromium fallback because the Browser Node REPL invocation timed out after 15 seconds. Playwright evidence is saved at `/tmp/one117-guardrails-runtime-report.json`; screenshots include `/tmp/one117-demo-empty.png`, `/tmp/one117-demo-success.png`, `/tmp/one117-live-missing-key.png`, `/tmp/one117-live-missing-consent.png`, `/tmp/one117-live-request-consent.png`, and `/tmp/one117-mobile-live-guardrail.png`. The runtime pass authenticated to `/trip-planner/`, confirmed page identity, nonblank content, no framework overlay, keyboard focus through prompt/destination/save/submit controls, demo `POST /wp-json/baf/v1/ai/itinerary` returning `201`, rendered demo output marked `Not sent to a live provider`, live missing-key and missing saved-consent guardrails with zero itinerary REST requests, live missing per-request consent with zero itinerary REST requests, no rendered fake API key, no app-owned console errors or failed requests, and no mobile horizontal overflow.

P18.6 local result on 2026-05-13: PHP syntax passed for `plugins/bookings-flights-core/includes/ai/class-itinerary-schema.php`, `plugins/bookings-flights-core/includes/ai/class-openai-provider.php`, `plugins/bookings-flights-core/includes/services/class-ai-itinerary-service.php`, `plugins/bookings-flights-core/includes/settings/class-settings-manager.php`, and `plugins/bookings-flights-core/includes/admin/class-admin-manager.php`; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`; `git diff --check` passed; changed source files remained below the 600-line guideline. `bookings-flights-core` deactivate/reactivate/is-active passed with known local WP-CLI PHP 8.5 deprecation noise only. Focused backend/privacy smoke confirmed `/baf/v1/ai/itinerary` and `/baf/v1/ai/handoff` have explicit permission callbacks, malformed itinerary output returns `baf_ai_output_missing_fields`, array-valued AI settings sanitize to demo/empty-provider/empty-key defaults, dashboard rendering does not expose a fake API key, provider errors return safe public messages, and AI session logs do not store fake secrets or raw prompt sentinels. No `Array to string conversion`, warning, fatal, fake-key, or raw-prompt leaks appeared in the smoke output. Browser validation attempted the Codex in-app Browser first, but no active browser pane was available, so Playwright Chromium was used. Runtime evidence is saved at `/tmp/one118-ai-review-runtime-report.json`; screenshots include `/tmp/one118-ai-review-empty.png`, `/tmp/one118-ai-review-success.png`, `/tmp/one118-ai-review-live-consent.png`, and `/tmp/one118-ai-review-mobile.png`. The final runtime pass confirmed page identity, nonblank planner content, no framework overlay, keyboard reachability through prompt/destination/submit, demo `POST /wp-json/baf/v1/ai/itinerary` returning `201`, rendered output avoiding the raw private prompt sentinel, demo output marked `Not sent to a live provider`, live missing per-request consent with zero additional itinerary REST requests, no fake API key in page source, no app-owned console errors, no app-owned failed requests, and no mobile horizontal overflow. Non-app WordPress/admin and Travelpayouts asset aborts were kept in the report as navigation noise.

P18.7 local result on 2026-05-13: PHP syntax passed for the Phase 18 AI, REST, service, frontend, template, settings, admin, and post-type files; `node --check` passed for `plugins/bookings-flights-core/assets/js/ai-planner.js`; `bookings-flights-core` deactivate/reactivate/is-active passed with known WP-CLI PHP 8.5 deprecation noise only; `curl` returned `200` for `/trip-planner/`. Focused WP-CLI backend smoke passed 34 assertions and saved `/tmp/one119-backend-smoke-report.json`: explicit itinerary/handoff permission callbacks, unauthenticated itinerary failure, demo draft save, no raw prompt echo/storage, run-only save failure, live missing key, unsupported provider, missing saved External AI consent, missing per-request AI consent, fake-key-safe errors, schema rejection for booked status, price claims, provider links, provider-link arrays, string false approval, string false disclosure, handoff missing nonce, limited-user handoff failure, missing approval, missing per-request provider consent, missing saved provider consent, local handoff success, malformed handoff parameter rejection, session privacy, and fixture cleanup. Browser validation attempted the Codex in-app Browser first, but the connection timed out after 15 seconds, so Playwright Chromium was used. Runtime evidence is saved at `/tmp/one119-phase18-final-report.json`; screenshots include `/tmp/one119-planner-empty.png`, `/tmp/one119-planner-saved.png`, `/tmp/one119-handoff-prepared.png`, `/tmp/one119-live-consent.png`, and `/tmp/one119-mobile.png`. The final runtime pass returned `status=pass` and `findingCount=0`, confirming page identity, nonblank planner content, no framework overlay, demo mode visible, no fake key in source, no desktop/mobile overflow, keyboard reachability through planner controls and handoff controls, itinerary `201`, handoff `201`, local `Provider action: not_executed`, no raw prompt/provider link/secret in saved draft or handoff meta, live missing per-request consent blocked with zero additional itinerary REST requests, no app-owned console errors, no app-owned failed requests, and cleanup of temporary browser user/draft content.
