...
P14.1 Codex review patch result on 2026-05-12: WP-CLI confirmed the stale stored menu returns `has_product_core=false`, renders all seven fallback labels, keeps the Trip Planner homepage anchor, and rejects external-host product URLs before a stored menu can bypass the fallback. Playwright rerun confirmed desktop navigation labels, zero horizontal overflow, desktop tab order through the product links/CTA/theme/search controls, mobile menu visibility for all seven product links, mobile menu keyboard loop, and no console warnings/errors or failed requests. Screenshots: `/tmp/one-86-host-patch-desktop.png`, `/tmp/one-86-host-patch-mobile-menu.png`. Runtime evidence: `/tmp/one-86-host-patch-runtime.json`.
