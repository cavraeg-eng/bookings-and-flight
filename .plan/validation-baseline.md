...
P19.2 local result on 2026-05-13: PHP syntax passed for the new alert service, alert handler, alert shortcode, job runner, cron manager, post-type registrar, and homepage template. `bookings-flights-core` deactivate/reactivate/is-active passed with known WP-CLI PHP 8.5 deprecation noise only. Focused WP-CLI smoke passed 35 assertions for initial alert save, duplicate email/route update, ten-alert per-email cap, eleventh-route rejection, queued email capture through `pre_wp_mail`, provider-boundary email copy, delete-link generation, signed token deletion, cron job email processing/status recording, shortcode lifecycle copy, nonce rendering, and temporary alert cleanup. A real missing-nonce `admin-post.php` request returned `403` and created zero alert records. New alert follow-up meta keys were confirmed registered for `travel_alert` with `show_in_rest=false`; the meta-registration probe emitted duplicate core block registration notices only because it manually invoked `init` inside WP-CLI after bootstrap. The Codex in-app Browser path was attempted first, but no active browser pane was available, so Playwright Chromium was used. Runtime evidence is saved at `/tmp/one121-runtime-review-report.json`; screenshots include `/tmp/one121-alert-form-desktop.png`, `/tmp/one121-alert-saved-desktop.png`, and `/tmp/one121-alert-form-mobile.png`. The final report returned `status=pass` and `findingCount=0`, confirming the alert form renders lifecycle/provider-boundary copy, real browser POST submission redirects to `baf_alert_status=saved`, the success state mentions queued confirmation/delete email, keyboard navigation reaches email/origin/destination/frequency/consent/submit, no app-owned console errors or failed requests were observed, no horizontal overflow was found, no secret-like source text was rendered, and temporary browser-created alerts were deleted.
